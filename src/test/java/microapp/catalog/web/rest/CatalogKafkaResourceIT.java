...
import microapp.catalog.config.EmbeddedKafka;
import microapp.catalog.config.KafkaSseConsumer;
import microapp.catalog.config.KafkaSseProducer;
...
class CatalogKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/catalog-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
